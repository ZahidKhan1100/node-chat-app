...
async function FriendsList(req, res) {
  try {
    ...
    const friendIds = friendRequests.reduce((ids, request) => {
      if (request.user2.toString() === req.userId) {
    ...
    const friendsDetails = await UserModel.find({
      _id: { $in: friendIds.map((friend) => friend.userId) },
    }).select("-password -email");

    // Combine friend details with status
    const friendsWithStatus = friendsDetails.map((friend) => {
      const { userId, status } = friendIds.find((f) =>
        f.userId.equals(friend._id)
      );
...
async function acceptRequest(req, res) {
  try {
    ...
    const result = await FriendsModel.updateOne(
      { user2: userId, user1: user1 },
      {
        $set: {
          status: "accepted",
        },
...
module.exports = { addFriend, FriendsList, acceptRequest };
